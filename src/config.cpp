...
Config load_config(const std::string &path, const std::string &cpu_override) {
  ...
  while (std::getline(input, line)) {
    ...
    else if (key == "runtime.dynamic") c.dynamic = boolean(value);
    else if (key == "runtime.static_quiescent_windows") c.static_quiescent_windows = std::stoi(value);
  ...
  if (c.sample_interval_seconds < 0 || c.graph_horizon_seconds < c.sample_interval_seconds ||
      ...
      c.minimum_confidence > 1 || c.static_quiescent_windows < 1 ||
